...
#include <stdint.h>///////////////////////AJOUTE***************************
#include <rc/gpio.h>///////////////////////AJOUTE***************************
#include "rc_balance_DS_defs.h"
...
typedef struct core_state_t{
	...
	bool sensorInfraMilieu; ///< Value of IR sensor
	bool sensorInfraDroite; ///< Value of IR sensor
	bool sensorInfraDroiteDroite; ///< Value of IR sensor
	bool sensorInfraGauche; ///< Value of IR sensor
	bool sensorInfraGaucheGauche; ///< Value of IR sensor
...
int main(int argc, char *argv[])
{
	...
	rc_gpio_init(3,17,GPIOHANDLE_REQUEST_INPUT); 	///init gpio input	
	rc_gpio_init(1,17,GPIOHANDLE_REQUEST_INPUT); 	///init gpio input
	rc_gpio_init(1,25,GPIOHANDLE_REQUEST_INPUT); 	///init gpio input
	rc_gpio_init(3,1,GPIOHANDLE_REQUEST_INPUT); 	///init gpio input
	rc_gpio_init(3,2,GPIOHANDLE_REQUEST_INPUT); 	///init gpio input
...
static void __balance_controller(void)
{
	...
	******************************************************************/
	...
	cstate.sensorInfraMilieu=rc_gpio_get_value(3,17);	
	cstate.sensorInfraGauche=rc_gpio_get_value(1,17);
	cstate.sensorInfraDroite=rc_gpio_get_value(1,25);
	cstate.sensorInfraGaucheGauche=rc_gpio_get_value(3,1);
	cstate.sensorInfraDroiteDroite=rc_gpio_get_value(3,2);
	bool Not_GG_DD = (cstate.sensorInfraDroiteDroite==1) || (cstate.sensorInfraGaucheGauche == 1);
	
	if ((cstate.sensorInfraGauche == 0) && (cstate.sensorInfraDroite == 0) && (cstate.sensorInfraMilieu == 1) && (Not_GG_DD ==0)) //Avancer tout droit
	{
		setpoint.phi_dot=5;
		setpoint.gamma_dot=0;
	}
	else if ((cstate.sensorInfraGauche == 1) && (cstate.sensorInfraDroite == 0) && (cstate.sensorInfraMilieu == 1)&& (Not_GG_DD ==0)) //Tourner gauche et avancer
	{
		setpoint.phi_dot=5;
		setpoint.gamma_dot=3;
	}
	else if ((cstate.sensorInfraGauche == 1) && (cstate.sensorInfraDroite == 0) && (cstate.sensorInfraMilieu == 0)&& (Not_GG_DD ==0)) //Grand virage gauche
	{
		setpoint.phi_dot=1;
		setpoint.gamma_dot=4;
	}
	else if ((cstate.sensorInfraGauche == 0) && (cstate.sensorInfraDroite == 1) && (cstate.sensorInfraMilieu == 1)&& (Not_GG_DD ==0)) //Tourner droite et avancer
	{
		setpoint.phi_dot=5;
		setpoint.gamma_dot=-3;
	}
	else if ((cstate.sensorInfraGauche == 0) && (cstate.sensorInfraDroite == 1) && (cstate.sensorInfraMilieu == 0)&& (Not_GG_DD ==0)) //grand virage droite
	{
		setpoint.phi_dot=1;
		setpoint.gamma_dot=-4;
	}
	else if ((cstate.sensorInfraGauche == 0) && (cstate.sensorInfraDroite == 0) && (cstate.sensorInfraMilieu == 0)&& (Not_GG_DD ==0)) //Arret zone blanche
	{
		setpoint.phi_dot=0;
		setpoint.gamma_dot=0;
	}
	else if ((cstate.sensorInfraGauche == 1) && (cstate.sensorInfraDroite == 1) && (cstate.sensorInfraMilieu == 1)&& (Not_GG_DD == 1)) //Arret Ligne Noir
	{
		setpoint.phi_dot=0;
		setpoint.gamma_dot=0;
	}
	else if ((cstate.sensorInfraGaucheGauche == 1) || (cstate.sensorInfraDroiteDroite == 0)) // Capteurs extremes ==> Reculer
	{
		setpoint.phi_dot=0;
		setpoint.gamma_dot=8;
	}
	else if ((cstate.sensorInfraGaucheGauche == 0) || (cstate.sensorInfraDroiteDroite == 1)) // Capteurs extremes ==> Reculer
	{
		setpoint.phi_dot=0;
		setpoint.gamma_dot=-8;
	}
...
static void* __printf_loop(__attribute__ ((unused)) void* ptr)
{
	...
	while(rc_get_state()!=EXITING){
		...
		if(new_rc_state == RUNNING){
			printf("\r");
			printf("%7.3d  |", cstate.sensorInfraGaucheGauche);
			printf("%7.3d  |", cstate.sensorInfraGauche);
			printf("%7.3d  |", cstate.sensorInfraMilieu);
			printf("%7.3d  |", cstate.sensorInfraDroite);
			printf("%7.3d  |", cstate.sensorInfraDroiteDroite);
