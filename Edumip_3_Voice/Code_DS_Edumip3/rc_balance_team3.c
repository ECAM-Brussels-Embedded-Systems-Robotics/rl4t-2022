...
#include "rc_balance_team3_defs.h"
...
typedef struct core_state_t{
	...
	double theta_a;    /// theta evalué à partir de l'accéléromètre
	double phi;		///< average wheel angle in global frame
	double phi_dot;   /// state observed
	double phi_dot_LY;  ///result of the reduces observer
	double phi_dot_LY_old; ///old value of the reduced observer result
...
m_input_mode_t m_input_mode = STDIN;
...
static void __balance_controller(void)
{
	...
	******************************************************************/
	...
	cstate.theta_a = -RAD2DEG*atan2(mpu_data.accel[2],mpu_data.accel[1]);

	cstate.theta = ((1-(TS/T))*(cstate.theta + TS * mpu_data.gyro[0]*DEG2RAD) + (TS/T)*cstate.theta_a*DEG2RAD) + BOARD_MOUNT_ANGLE * 0.01 ;
	
	dutyT = dutyL+dutyR;
	cstate.phi_dot_LY_old = cstate.phi_dot_LY;
	cstate.phi_dot_LY = M1*cstate.phi_dot + (m2[0] * cstate.theta + m2[1] * mpu_data.gyro[0] + m2[2]*cstate.phi)+ M3 * dutyT ; 
	cstate.phi_dot = cstate.phi_dot_LY_old + (l[0] * cstate.theta + l[1]* mpu_data.gyro[0] + l[2] * cstate.phi);
...
void conn_TCP(void)
{
	...
	const char order[1] = "1";
	const char order1[1] = "2";
	const char order2[1] = "3";
	const char order3[1] = "4";
	const char order4[1] = "5";
	const char order5[1] = "6";
    ...
    servaddr.sin_addr.s_addr = inet_addr("192.168.0.230");
	...
	else if(buff[0]==order1[0]){
		setpoint.phi_dot = -5;
		setpoint.phi += setpoint.phi_dot*DT;
	}
	else if(buff[0]==order2[0]){
		setpoint.phi_dot = 0;
		setpoint.gamma_dot = 0;
	}
	else if(buff[0]==order3[0]){
		setpoint.gamma_dot = 3;
		setpoint.gamma += setpoint.gamma_dot*DT;
	}
	else if(buff[0]==order4[0]){
		setpoint.gamma_dot = -3;
		setpoint.gamma += setpoint.gamma_dot*DT;
	}
	else if(buff[0]==order5[0]){
		setpoint.gamma_dot = 0;
	}
	else{
		setpoint.phi_dot = 0;
