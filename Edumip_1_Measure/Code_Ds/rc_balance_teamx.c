...
#include "rc_balance_teamx_defs.h"
...
int acceptConnections(int udpSocketFileDescriptor);
...
int main(int argc, char *argv[])
{
    ...
    serverSocketAddress.sin_port = htons(5005);
	...
	rc_pthread_create (&test_t,acceptConnections,udpSocketFileDescriptor,SCHED_OTHER, 0);
...
int acceptConnections(int udpSocketFileDescriptor) {
	...
	while(TRUE){
		...
		int maxFileDescriptor = udpSocketFileDescriptor;
		...
		if ( FD_ISSET(udpSocketFileDescriptor, &readFileDescriptorSet) ) {
			...
			cstate.angle = atof(inputBuffer);
			cstate.angle = cstate.angle + cstate.gamma;
			fprintf(stderr, "[INFO][UDP] Received an angle from client %s:%d: %s\n", 
			...
			readBytes = recvfrom(udpSocketFileDescriptor, inputBuffer, BUFFER_SIZE, 0, (struct sockaddr *)(&clientSocketAddress), &sockaddrSize);
			cstate.destination = atof(inputBuffer);
			cstate.destination = cstate.destination + cstate.phi;
			
			// Receive a destination from client
			fprintf(stderr, "[INFO][UDP] Received a destination from client %s:%d: %s\n", 
				inet_ntoa(clientSocketAddress.sin_addr), ntohs(clientSocketAddress.sin_port), inputBuffer);
			while(TRUE){
				sleep(0.5);
				if (cstate.phi<cstate.destination){
					sleep(0.5);
				}
				else {
					sleep(2);
					sprintf(outputBuffer,"%f",cstate.phi);
					if ( sendto(udpSocketFileDescriptor, outputBuffer, strlen(outputBuffer), 0, (struct sockaddr *)(&clientSocketAddress), sockaddrSize) == -1 ) {
					fprintf(stderr, "[ERROR][UDP] An error occurred while sending message to the client %s:%d: %s\nThe connection is going to close.\n", 
						inet_ntoa(clientSocketAddress.sin_addr), ntohs(clientSocketAddress.sin_port), strerror(errno));
					}
...
{
	if(cstate.gamma<(cstate.angle-0.05) || cstate.gamma>(cstate.angle+0.05)){
		setpoint.gamma = cstate.angle;
	}
	else {
		if ((cstate.phi - cstate.destination)<-1) {
			setpoint.phi_dot = 10;
		}
		else{
			setpoint.phi_dot = 0;
	...
	double K_array[]=K_co;
	cstate.someturn = mpu_data.dmp_TaitBryan[TB_YAW_Z];
	...
	******************************************************************/
	...
	//theta de l'ancien algo
	cstate.theta_b = mpu_data.dmp_TaitBryan[TB_PITCH_X] + BOARD_MOUNT_ANGLE;

	//angle theta avec l'algo complémentaire
	...
	//Phi dot observateur
	cstate.phi_dot = (L_array[0]*cstate.theta + L_array[1]*cstate.theta_dot + L_array[2]*cstate.phi) + cstate.phi_dot_memo;
	...
	***********************************************************/
	...
	//observateur phi_dot
	cstate.phi_dot_memo = (A22_LA12)*cstate.phi_dot + (A21_LA11_array[0]*cstate.theta + A21_LA11_array[1] * cstate.theta_dot + A21_LA11_array[2]*cstate.phi) + (B2_LB1)*(dutyL+dutyR);
